...
def create_nat_set(nat_list):
    all_nats_set = set()  # duplicado ou nao - tutpla com porta e ip
    all_nats_obj = {}
...
def update_marked_with_inputs(input_nats, all_nats):
    ...
    for nat in all_nats:
        ...
        if 'mark' in nat and nat["mark"] == mark_dnat:
            ...
            result_compare.append(nat)
    return {"result_compare": result_compare, "max_port": port, "advertise_ip": advertise_ip, "exclude": exclude}
...
if not deduplicated_nats and updated_mark_nats["exclude"] == False:
    #nochanges = 1
    print(f"\033[1;32m ===================================================================== \033[0m")
    print("\033[1;32m |PLAN| -> NENHUMA ALTERAÇÃO SERÁ REALIZADA! NO CHANGES! \033[0m")    
